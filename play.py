...
def heuristic():
    ...
    if len(bestStart) != 0:
        bestStart.sort(
            key=lambda x: mancalaBoard.board[getFinishPoint(x)], reverse=True)
        return bestStart[0]  # returns the pit with the most seeds
    else:  # search for a move that ends in a computer pit and that's it
        goodStart = []
        for move in mancalaBoard.possibleMoves(2):
        ...
        if len(goodStart) != 0:
            goodStart.sort(key=lambda x: mancalaBoard.board[x], reverse=True)
            return goodStart[0]
            ...
            if len(mehStart) != 0:
                mehStart.sort(
                    key=lambda x: mancalaBoard.board[x], reverse=True)
                return mehStart[0]
            else:  # just search for a random move when can't find a good one
                badStart = []
                for move in mancalaBoard.possibleMoves(2):
...
def humanTurn():
    for x in circlesPos:
        if circleClicked(circlesPos[x][1]):
            mancalaBoard.board, mancalaBoard.turn = (mancalaBoard.doMove(x, 1))
            SFX.play(mixer.Sound(
                SFX_COLLECTION[random.randint(0, len(SFX_COLLECTION)-1)]))


def computerTurn():
    startPoint = heuristic()
...
slotPos = {'A': list(zip([random.randint(156 - (circlesRadius - 60), 156 + (circlesRadius - 25)*2) for _ in range(44)], [random.randint(272 + (10), 272 + (circlesRadius - 12)*2) for _ in range(44)])),
           ...
           'store2': list(zip([random.randint(40, 80) for _ in range(44)], [random.randint(130, 126 + (circlesRadius - 12)*6) for _ in range(44)]))
           }
...
if __name__ == '__main__':
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if not over and event.type == pygame.MOUSEBUTTONDOWN and mancalaBoard.turn == 1:
                humanTurn()
